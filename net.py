...
class DSIN_layer(nn.Layer):
    def __init__(self, user_size, adgroup_size, pid_size, cms_segid_size, cms_group_size,
        ...
        self.transform_actpool = AttentionSequencePoolingLayer(weight_normalization=True)
        self.lstm_actpool = AttentionSequencePoolingLayer(weight_normalization=True)

        # MLP moudle
        self.mlp = MLP(mlp_hidden_units=[84, 64, 16])

    def _get_weight_attr(self):
        return paddle.ParamAttr(initializer=nn.initializer.TruncatedNormal(std=.02))
    ...
    def forward(self,inputs):
        ...
        lstm_output = lstm_output[:, :, :2*self.sparse_embed_size] + lstm_output[:, :, 2*self.sparse_embed_size:]
        ...
        mlp_input = paddle.concat([deep_input_embeded, query_embeded, paddle.nn.Flatten()(lstm_input), paddle.nn.Flatten()(lstm_output)], axis=-1)
